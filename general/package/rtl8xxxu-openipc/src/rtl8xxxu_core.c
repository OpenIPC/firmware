...
void rtl8xxxu_gen1_config_channel(struct ieee80211_hw *hw)
{
	...
	case NL80211_CHAN_WIDTH_40:
		if (rtl8xxxu_conf_center_freq1(hw) >
	...
	for (i = RF_A; i < priv->rf_paths; i++) {
		val32 = rtl8xxxu_read_rfreg(priv, i, RF6052_REG_MODE_AG);
		if (rtl8xxxu_conf_width(hw) == NL80211_CHAN_WIDTH_40)
...
static void rtl8xxxu_set_basic_rates(struct rtl8xxxu_priv *priv, u32 rate_cfg)
{
	...
	val32 = rtl8xxxu_read32(priv, REG_RESPONSE_RATE_SET);
	if (hw->conf.channel->band == NL80211_BAND_5GHZ)
...
static void rtl8xxxu_set_aifs(struct rtl8xxxu_priv *priv, u8 slot_time)
{
	...
	if (priv->hw->conf.channel->band == NL80211_BAND_5GHZ ||
				      ...
				      u8 rate, u8 sgi, u8 bw)
{
	...
	if (rate <= DESC_RATE_54M) {
		...
		/* 3.4.43 struct rate_info has no nss field */

		if (sgi)
			rarpt->txrate.flags |= RATE_INFO_FLAGS_SHORT_GI;

		if (bw == RATE_INFO_BW_40)
			rarpt->txrate.flags |= RATE_INFO_FLAGS_40_MHZ_WIDTH;
	}
...
static void rtl8xxxu_c2hcmd_callback(struct work_struct *work)
{
	...
	case C2H_8723B_RA_REPORT:
		bw = (rarpt->txrate.flags & RATE_INFO_FLAGS_40_MHZ_WIDTH) ?
...
static int rtl8xxxu_config(struct ieee80211_hw *hw, u32 changed)
{
	...
	if (rtl8xxxu_debug & RTL8XXXU_DEBUG_CHANNEL)
		dev_info(dev,
			 ...
			 changed, rtl8xxxu_conf_width(hw));
			    ...
			    const struct ieee80211_tx_queue_params *param)
{
	...
	bool acm = false; /* 3.4.43 tx_queue_params has no acm field */
	...
	dev_dbg(dev,
		...
		__func__, queue, val32, acm, acm_ctrl);
		      ...
		      struct ieee80211_sta *sta, u16 tid, u16 *ssn, u8 buf_size)
{
	...
	case IEEE80211_AMPDU_TX_START:
		...
		/* 3.4.43: driver must start the BA session itself */
...
#if 0 /* dropped: sta_statistics not in 3.4.43 ops */
static void
...
#endif /* dropped: sta_statistics not in 3.4.43 ops */
				       ...
				       int signal, struct ieee80211_sta *sta)
{
	...
	txbw_40mhz = (rtl8xxxu_conf_width(hw) == NL80211_CHAN_WIDTH_40) ? 1 : 0;
			  ...
			  const struct usb_device_id *id)
{
	...
	/* IEEE80211_HW_SUPPORT_FAST_XMIT not present in 3.4.43 */
...
/* RTL8188FU only — other chip fops not built in this backport */
