...
static void rtl8188fu_config_channel(struct ieee80211_hw *hw)
{
	...
	val32 = rtl8xxxu_read32(priv, REG_FPGA0_RF_MODE);
	val32 &= ~FPGA_RF_MODE;
	val32 |= rtl8xxxu_conf_width(hw) == NL80211_CHAN_WIDTH_40;
	rtl8xxxu_write32(priv, REG_FPGA0_RF_MODE, val32);
	...
	val32 &= ~GENMASK(23, 20);
	val32 |= BIT(21);
	if (rtl8xxxu_conf_width(hw) == NL80211_CHAN_WIDTH_20 ||
	    rtl8xxxu_conf_width(hw) == NL80211_CHAN_WIDTH_20_NOHT)
		val32 |= BIT(20);
	else if (rtl8xxxu_conf_width(hw) == NL80211_CHAN_WIDTH_40)
		val32 |= BIT(22);
	rtl8xxxu_write32(priv, REG_OFDM_RX_DFIR, val32);
...
struct rtl8xxxu_fileops rtl8188fu_fops = {
	...
	.phy_init_antenna_selection = NULL, /* 8188f: no separate antenna sel (was rtl8723bu_phy_init_antenna_selection, not built) */
